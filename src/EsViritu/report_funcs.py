...
def make_dash_aggrid_html_report(windows: 'pl.DataFrame', meta: 'pl.DataFrame', output_html: str = 'report.html') -> str:
    ...
    try:
        ...
        logger.error(f"Error processing dataframes for report: {e}")
        raise
    ...
    try:
        # First try to use Dash's built-in HTML generation capabilities
        # This preserves interactivity when possible
        try:
            logger.info("Attempting to use Dash's built-in HTML generation")

            # Add necessary scripts and dependencies for interactivity
            ...
            # Try to use Dash's internal methods to generate HTML with interactivity
            with app.server.app_context():
                # Get the index string from Dash
                index_string = '''
<!DOCTYPE html>
<html>
    <head>
    ...
    <body>
        <h1 style="color: #2c3e50; margin-bottom: 5px; text-align: center;">Virus Coverage Report</h1>
        <p style="color: #7f8c8d; margin-top: 0; text-align: center;">Generated: ''' + timestamp + '''</p>
        
        <div id="react-entry-point">
            {%app_entry%}
        </div>
        
        <div class="footer" style="margin-top: 30px; text-align: center; color: #7f8c8d; font-size: 0.9em; border-top: 1px solid #eee; padding-top: 20px;">
            <p>Report generated by EsViritu. <span style="font-weight: bold;">Coverage profiles</span> show normalized coverage across the genome.</p>
        </div>
...
'''
                ...
                # Generate HTML with Dash's renderer
                rendered = app.index_string = index_string
                html_content = flask.render_template_string(
                    app.index_string,
                    app_entry=app._generate_scripts_html(),
                ...
                logger.info("Successfully generated interactive HTML with Dash")
                
        except Exception as dash_error:
            # If Dash's built-in HTML generation fails, fall back to direct HTML generation
            logger.warning(f"Dash HTML generation failed, falling back to direct HTML: {dash_error}")
...
'''
            logger.info("Generated static HTML as fallback")
