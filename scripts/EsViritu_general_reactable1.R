...
coverage_bins_data <- coverage_data %>% 
...
sum_coverage <- coverage_bins_data %>%
  group_by(accession) %>%
  summarize(coverage = list(round)) 
...
combined_data <- subset(combined_data, select = c("sequence_name", "accession", "reference_length", "Percent_covered", "RPKM", "reads_aligned", "genus", "species", "subspecies", "coverage"))
